...
export default function HumanVerification({
    ...
    handleChoice: (choice: 'verify' | 'refuse') => void
}) {
        ...
        <IDKitWidget
            ...
            {({ open }) => (
                <button
                    ...
                    className='relative h-[64px] w-[272px] text-lg uppercase text-black'>
                    <ButtonSVG1 isHovered={isHovered} />
                    <span
                        className={`relative z-10 flex h-full items-center justify-center ${isHovered ? 'text-[#90FE74]' : ''}`}>
